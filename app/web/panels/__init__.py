"""W2's control panels: State and Proposals (roadmap section 4).
...
`setup_web` has one call to make and a later milestone (W3 memory, W4
check-in, W5 research) only has to add one line here, not touch
`setup_web` itself.
...
check_panel_write()`, then core calls only -- `app/core/commands.py` and
`app/core/proposal.py`, never a bare SQL write against `user_state` or
`proposal` -- and finally `hub.publish_invalidate(...)`. See each
