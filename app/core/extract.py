...
That is enforced structurally, not by care. This module imports exactly
three things that can write: `memory.write_memory`, `proposal.create`,
and a `Journal` row. It does not import `update_state`, and the only
name it holds from app/core/proposal.py is `create` -- `accept()`, the
single function that can touch a sensitive field, is not in scope here
and is reachable only from a button press. tests/test_extract.py
asserts that by source inspection as well as by behaviour, because a
future edit could add the import back without any test noticing
otherwise.
...
PROPOSAL_FIELDS = (proposal.DUE_ACTION, proposal.FOCUS_ON)
...
class ExtractOutcome:
    ...
    __slots__ = ("memories", "created", "expired")
