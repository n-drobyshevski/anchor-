...
- *On `main` (rev. 4):* this fix is superseded. `main` already protects an adopted technique from `/forget` (`FORGET_PROTECTED`, app/core/memory.py) and relinks cards to a successor, so no `forgotten` status exists; `forget_lineage` (8c) must honour that protection. The original text follows.
